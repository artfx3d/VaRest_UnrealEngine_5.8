...
{
	using KeyType = decltype(JsonObj->Values)::KeyType;

	TArray<KeyType> RawKeys;
	JsonObj->Values.GetKeys(RawKeys);
	TArray<FString> Result;
	Result.Reserve(RawKeys.Num());
	for (const KeyType& Key : RawKeys)
	{
		Result.Add(FString(Key));
	}
	return Result;
}
